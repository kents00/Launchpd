...
describe('auth commands', () => {
  ...
  describe('resendVerification', () => {
    ...
    it('should fail if not logged in', async () => {
      ...
      expect(info).toHaveBeenCalledWith(expect.stringContaining('Run "launchpd login"'))
    })

    it('should handle API failure', async () => {
      vi.mocked(credentials.isLoggedIn).mockResolvedValue(true)
      vi.mocked(api.resendVerification).mockResolvedValue({ success: false, message: 'Server error' })

      await resendEmailVerification()
      expect(spinner().fail).toHaveBeenCalledWith('Failed to send verification email')
    })

    it('should handle already verified error', async () => {
      vi.mocked(credentials.isLoggedIn).mockResolvedValue(true)
      vi.mocked(api.resendVerification).mockRejectedValue(new Error('Email already verified'))

      await resendEmailVerification()
      expect(success).toHaveBeenCalledWith(expect.stringContaining('already verified'))
    })
  })
  ...
  describe('quota', () => {
    it('should show anonymous limits if not logged in', async () => {
      ...
      expect(log).toHaveBeenCalledWith(expect.stringContaining('Anonymous Quota Status'))
      expect(log).toHaveBeenCalledWith(expect.stringContaining('3 maximum'))
    })

    it('should fetch and show quota if logged in', async () => {
      vi.mocked(credentials.getCredentials).mockResolvedValue({ apiKey: 'lpd_1234567890123456', email: 'test@example.com' })
      fetch.mockResolvedValueOnce({
        ok: true,
