...
describe('versions command', () => {
    ...
    it('should list versions from API', async () => {
        ...
        vi.mocked(api.getVersions).mockResolvedValue({
            versions: [
                { version: 2, created_at: new Date().toISOString(), file_count: 5, total_bytes: 1024, message: 'v2' },
                { version: 1, created_at: new Date().toISOString(), file_count: 5, total_bytes: 1024, message: 'v1' }
            ],
            activeVersion: 2
        })
        ...
        expect(logger.success).toHaveBeenCalledWith(expect.stringContaining('Versions for test-site'))
    ...
    it('should fail if not logged in', async () => {
        ...
        expect(logger.errorWithSuggestions).toHaveBeenCalledWith(expect.stringContaining('only available for authenticated'), expect.anything(), expect.anything())
    })
    ...
    it('should fallback to metadata if API returns null', async () => {
        ...
        vi.mocked(metadata.getVersionsForSubdomain).mockResolvedValue([
            { version: 1, timestamp: new Date().toISOString(), fileCount: 1, totalBytes: 100 }
        ])
    ...
    it('should handle no deployments found', async () => {
        ...
        await expect(versions('test-site', {})).rejects.toThrow('Process.exit(1)')
        expect(logger.spinner().fail).toHaveBeenCalledWith(expect.stringContaining('No deployments found'))
    })
    ...
    it('should warn about --to usage', async () => {
        ...
        expect(logger.warning).toHaveBeenCalledWith(expect.stringContaining('--to option is for the rollback'))
    })
})
