...
describe('Upload Utility', () => {
    ...
    describe('HMAC Signature', () => {
        it('should add X-Signature header if API secret is present', async () => {
            readdir.mockResolvedValue([{ isFile: () => true, name: 'index.html', path: '/test' }]);
            ...
            const fetchCalls = fetch.mock.calls.filter(call => call[0].includes('upload/file'));
    ...
    describe('Error Parsing', () => {
        it('should parse JSON error response', async () => {
            ...
            fetch.mockResolvedValueOnce({
                ...
                text: () => Promise.resolve(JSON.stringify({ error: 'Custom JSON Error' }))
            });

            await expect(uploadFolder('/test', 'mysite', 1)).rejects.toThrow('Custom JSON Error');
        });
        ...
        it('should parse Text error response', async () => {
            ...
            await expect(uploadFolder('/test', 'mysite', 1)).rejects.toThrow('Custom Text Error');
        });
    });
});
