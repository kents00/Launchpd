...
describe('versions command', () => {
    ...
    it('should exit with error if user is not logged in', async () => {
        ...
        vi.spyOn(credentials, 'isLoggedIn').mockResolvedValue(false);
    ...
    it('should proceed if user is logged in', async () => {
        ...
        vi.spyOn(credentials, 'isLoggedIn').mockResolvedValue(true);
        // Mock API response to avoid errors during execution
        vi.spyOn(api, 'getVersions').mockResolvedValue({ versions: [] });
        vi.spyOn(metadata, 'getVersionsForSubdomain').mockResolvedValue([]);
