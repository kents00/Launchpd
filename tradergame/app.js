...
class GameEngine {
    ...
    generateHistory() {
        ...
        this.lastCandle = { ...last, time: last.time + 60, open: last.close, high: last.close, low: last.close, close: last.close };
    ...
    bindEvents() {
        // UI Side Selection
        document.getElementById('side-buy').addEventListener('click', () => this.setSide('BUY'));
        document.getElementById('side-sell').addEventListener('click', () => this.setSide('SELL'));
        ...
        document.getElementById('tp-toggle').addEventListener('change', (e) => {
            ...
            if (this.tpEnabled && !this.tpPrice) {
                ...
                this.tpPrice = (this.orderSide === 'BUY' ? this.price + distance : this.price - distance);
            }
        ...
        document.getElementById('sl-toggle').addEventListener('change', (e) => {
            ...
            if (this.slEnabled && !this.slPrice) {
                ...
                this.slPrice = (this.orderSide === 'BUY' ? this.price - distance : this.price + distance);
            }
            this.ui.updateExitInputs(this);
            this.updateOverlays();
        });
        ...
        // Execution
        document.getElementById('btn-execute-trade').addEventListener('click', () => this.executeTrade());
        document.getElementById('btn-close').addEventListener('click', () => this.closePosition());
    ...
    simulateMarket() {
        const change = (Math.random() - 0.5) * CONFIG.volatility * (Math.random() > 0.95 ? 5 : 1);
        this.price += change;
        ...
        if (Math.random() > 0.98) {
            this.lastCandle = {
                time: candle.time + 60,
                open: this.price, high: this.price, low: this.price, close: this.price
            };
        }
    ...
    checkTriggers() {
        ...
        if (this.position) {
            ...
            if (tp) {
                if (side === 'BUY' && this.price >= tp) close = true, reason = 'TP';
                if (side === 'SELL' && this.price <= tp) close = true, reason = 'TP';
            }
            if (sl) {
                if (side === 'BUY' && this.price <= sl) close = true, reason = 'SL';
                if (side === 'SELL' && this.price >= sl) close = true, reason = 'SL';
            }
    ...
    executeTrade() {
        if (this.orderType === 'market') {
            this.openPosition(this.orderSide, this.orderMargin, this.tpEnabled ? this.tpPrice : null, this.slEnabled ? this.slPrice : null);
        } else {
            const order = {
                id: Date.now(),
                side: this.orderSide,
            ...
            this.chart.addMarker(this.lastCandle.time, this.orderSide === 'BUY' ? 'buy' : 'sell', `${this.orderType.toUpperCase()} @ ${this.orderPrice.toFixed(2)}`);
        }
        this.updateOverlays();
    }
    ...
    openPosition(side, margin, tp = null, sl = null, price = null) {
        ...
        this.chart.addMarker(this.lastCandle.time, side === 'BUY' ? 'buy' : 'sell', `Entry @ ${entryPrice.toFixed(2)}`);
        this.updateOverlays();
    }
    ...
    closePosition(reason = '') {
        ...
        this.chart.addMarker(this.lastCandle.time, this.position.side === 'BUY' ? 'sell' : 'buy', `${reason || 'Close'} @ ${this.price.toFixed(2)} (${(pnl >= 0 ? '+' : '')}${pnl.toFixed(2)})`);
    ...
    updateOverlays() {
        ...
        const units = (this.orderMargin * 10) / (this.orderType === 'market' ? this.price : this.orderPrice);
        const pricesToFit = [];
        ...
        if (this.tpEnabled) {
            ...
            const pnl = (this.tpPrice - (this.orderType === 'market' ? this.price : this.orderPrice)) * units * (this.orderSide === 'BUY' ? 1 : -1);
            ...
            const basePrice = this.orderType === 'market' ? this.price : this.orderPrice;
            this.chart.priceManager.addShadedZone('tp_zone_active', basePrice, this.tpPrice, 'profit-zone');
        }

        // 3. SL line (Active Config)
        if (this.slEnabled) {
            pricesToFit.push(this.slPrice);
            const pnl = (this.slPrice - (this.orderType === 'market' ? this.price : this.orderPrice)) * units * (this.orderSide === 'BUY' ? 1 : -1);
            ...
            const basePrice = this.orderType === 'market' ? this.price : this.orderPrice;
            this.chart.priceManager.addShadedZone('sl_zone_active', basePrice, this.slPrice, 'loss-zone');
        }
        ...
        this.pendingOrders.forEach(ord => {
            ...
            if (ord.tp) {
                const pnl = (ord.tp - ord.price) * ordUnits * (ord.side === 'BUY' ? 1 : -1);
                this.chart.priceManager.addLine(`pending_tp_${ord.id}`, ord.tp, {
                    color: '#26a69a', lineStyle: 1, title: 'TP', type: 'TAKE PROFIT',
                    pnl: `${pnl >= 0 ? '+' : ''}${pnl.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD`
                });
            }
            if (ord.sl) {
                const pnl = (ord.sl - ord.price) * ordUnits * (ord.side === 'BUY' ? 1 : -1);
                this.chart.priceManager.addLine(`pending_sl_${ord.id}`, ord.sl, {
                    color: '#ef5350', lineStyle: 1, title: 'SL', type: 'STOP LOSS',
                    pnl: `${pnl >= 0 ? '+' : ''}${pnl.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD`
                });
            }
        });
        ...
        if (this.position) {
            ...
            if (tp) {
                this.chart.priceManager.addShadedZone('pos_tp_zone', entryPrice, tp, 'profit-zone');
                const pnl = (tp - entryPrice) * posUnits * (side === 'BUY' ? 1 : -1);
                this.chart.priceManager.addLine('pos_tp', tp, {
                    color: '#26a69a', lineStyle: 1, title: 'TP', type: 'TAKE PROFIT',
                    pnl: `${pnl >= 0 ? '+' : ''}${pnl.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD`,
                    onDrag: (p) => { this.position.tp = p; this.updateOverlays(); }
                });
            }
            if (sl) {
                this.chart.priceManager.addShadedZone('pos_sl_zone', entryPrice, sl, 'loss-zone');
                const pnl = (sl - entryPrice) * posUnits * (side === 'BUY' ? 1 : -1);
                this.chart.priceManager.addLine('pos_sl', sl, {
                    color: '#ef5350', lineStyle: 1, title: 'SL', type: 'STOP LOSS',
                    pnl: `${pnl >= 0 ? '+' : ''}${pnl.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD`,
                    onDrag: (p) => { this.position.sl = p; this.updateOverlays(); }
                });
            }
        }
...
class ChartManager {
    constructor(containerId) {
        ...
        this.priceManager = new PriceLineManager(this.chart, this.candleSeries, 'chart-overlay-labels');
        ...
        this.chart.timeScale().subscribeVisibleLogicalRangeChange(() => this.priceManager.sync());
        this.chart.priceScale('right').applyOptions({ autoScale: true });
    }

    setData(data) { this.candleSeries.setData(data); }
    update(candle) {
    ...
    addMarker(time, type, text) {
        ...
        if (isBuy) {
            ...
            color = '#ef5350'; shape = 'arrowDown'; position = 'aboveBar';
        }
...
class PriceLineManager {
    ...
    sync() {
        this.lines.forEach((l, id) => {
            ...
            if (y !== null) {
                ...
                if (y > containerRect.height - 20) l.element.style.top = `${containerRect.height - 20}px`;
            } else {
    ...
    onMouseMove(e) {
        ...
        if (price) {
            ...
            l.price = price;
            l.line.applyOptions({ price: price });
...
class UIManager {
    ...
    update(game) {
        ...
        this.els.pnl.innerText = (openPnl >= 0 && openPnl !== 0 ? '+' : '') + this.fmt(openPnl);
        ...
        const winRate = game.stats.trades > 0 ? (game.stats.wins / game.stats.trades * 100).toFixed(0) : 0;
    ...
    updateOrderLabels(game) {
        const units = (game.orderMargin * 10) / (game.orderType === 'market' ? game.price : game.orderPrice);
    ...
    updatePrice(p) { this.els.inputPrice.value = p.toFixed(2); }
    updateTP(p) { this.els.tpInput.value = p.toFixed(2); }
    updateSL(p) { this.els.slInput.value = p.toFixed(2); }

    setTPEnabled(val) { this.els.tpToggle.checked = val; this.els.tpWrap.classList.toggle('disabled', !val); this.els.tpInput.disabled = !val; }
    setSLEnabled(val) { this.els.slToggle.checked = val; this.els.slWrap.classList.toggle('disabled', !val); this.els.slInput.disabled = !val; }
    ...
    fmt(num) { return '$' + num.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'); }
