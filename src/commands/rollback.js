...
export async function rollback (subdomainInput, options) {
  ...
  try {
    ...
    if (options.to) {
      ...
      const versionExists = versions.some(function (v) {
      ...
      if (!versionExists) {
        ...
        for (const v of versions) {
          ...
          log(
            '  ' +
              chalk.cyan(`v${v.version}`) +
              message +
              ' - ' +
              chalk.gray(v.timestamp) +
              marker
          )
        }
        process.exit(1)
    ...
    if (targetVersion === currentActive) {
      warning(
        'Version ' + chalk.cyan(`v${targetVersion}`) + ' is already active.'
      )
      process.exit(0)
    }
    ...
    rollbackSpinner.succeed(
      `Rolled back to ${chalk.cyan(`v${targetVersion}`)}`
    )
