...
export function promptSecret (question) {
  return new Promise((resolve) => {
    ...
    /**
     * Handle keypress events for secret input
     * @param {string} str - The character pressed
     * @param {Object} key - Key metadata (name, ctrl, meta, etc.)
     */
    const handler = (str, key) => {
      ...
      if (str?.length === 1 && /[ -~]/.test(str) && !key.ctrl && !key.meta) {
        secret += str
        stdout.write('*')
      }
    }
    ...
    /**
     * Clean up stdin state and remove event listeners
     */
    function cleanup () {
      if (stdin.isTTY) {
